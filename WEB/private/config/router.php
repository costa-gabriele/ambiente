...
if(!file_exists(ROUTES_CACHE_PATH)) {require ROUTES_SETUP_PATH;}
...
if(!empty($destinationInfo['path']) && file_exists($destinationInfo['path'])) {
	...
	$fileContentType = finfo_file(finfo_open(FILEINFO_MIME_TYPE), $destinationInfo['path'], FILEINFO_MIME_TYPE);
	...
	if($fRequire)
		...
		echo file_get_contents($destinationInfo['path']);
